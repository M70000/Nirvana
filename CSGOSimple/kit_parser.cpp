...
auto initialize_kits() -> void
{
	...
	// Dump sticker kits
	{
		const auto sticker_sig = Utils::PatternScan(GetModuleHandle(L"client.dll"), "53 8D 48 04 E8 ? ? ? ? 8B 4D 10") + 4;
		...
		for (auto i = 0; i <= map_head->last_element; ++i)
		{
			...
			if (strstr(sticker_name_ptr, "StickerKit_dhw2014_dignitas"))
			{
				...
				sticker_name_ptr = sticker_name_if_valve_fucked_up_their_translations; //HAHA LUL DA FUCK, credits @namazso
			}
