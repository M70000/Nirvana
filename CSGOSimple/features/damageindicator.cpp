...
void DamageIndicators::paint() {
	...
	for (int i = 0; i < floatingTexts.size(); i++)
	{
		...
		char msg[12];
		if (g_Options.esp_damageindremainhp)
		...
		if (txt->damage >= 66 && txt->damage <= 800)
			damage_color = Color(255, 255, 255, 255);
		...
		{
			Render::Get().RenderText(msg, ImVec2(origin_screen.x, origin_screen.y), 15.f, Color(g_Options.color_damage_indicator), false, true, g_pTahomaFont);
		}
		else
		{
			Render::Get().RenderText(msg, ImVec2(origin_screen.x, origin_screen.y), 15.f, Color(g_Options.color_damage_indicator), false, true, g_pSmallestPixelFont);
		}
	}
}

void DamageIndicators::RemoveListener()
{
	g_GameEvents->RemoveListener(&player_hurt_listener);
	g_GameEvents->RemoveListener(&bullet_impact_listener);
}
