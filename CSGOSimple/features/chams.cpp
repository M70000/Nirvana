...
void Chams::OnDrawModelExecute( void * pResults, DrawModelInfo_t * pInfo, matrix3x4_t * pBoneToWorld,
  ...
  static auto fnDME = Hooks::draw_model_execute::original;
  ...
  if( g_Options.chams_player_enabled && strstr( mdl->szName, "models/player" ) != nullptr ) {
	 ...
	 if( ent && ent->IsPlayer( ) && ent->IsAlive( ) ) {
		...
		if( g_Options.toggle_chams_history && g_Backtrack.data.count( ent->EntIndex( ) ) > 0 ) {
		  auto& data = g_Backtrack.data.at( ent->EntIndex( ) );
		  if( data.size( ) > 0 ) {
			 if( g_Options.chams_backtrack == 0 ) 
			 {
				 auto& back = data.back();
				 if (g_Options.chams_backtrack_type == 0)
				 {
					 OverrideMaterial(false, false, false, false, Color(g_Options.color_chams_backtrack));
					 //Cool stuff /*Color(255 - (i * (255 / data.size())), i * (255 / data.size()), 255, 30)*/
				 }
				 else
				 {
					 OverrideMaterial(false, true, false, false, Color(g_Options.color_chams_backtrack));
				 }
