...
void Backtrack::OnMove( CUserCmd *pCmd ) {
	...
	if (g_Aimbot.settings.backtrack_time == 0.f)
	{
		data.clear();
		return;
	}
...
void Backtrack::AddLatencyToNetwork(INetChannel* network, float latency) noexcept
{
	CUserCmd* cmd;
	for (auto& sequence : sequences)
	{
		if (serverTime(cmd) - sequence.servertime >= latency)
		{
...
void Backtrack::UpdateIncomingSequences(bool reset) noexcept
{
	...
	if (!g_Options.fakelatency)
		return;

	if (!g_LocalPlayer)
		return;

	auto network = g_NetChannel;
	if (!network)
		return;
	...
	if (network->m_nInSequenceNr > lastIncomingSequenceNumber)
	{
		...
		sequence.servertime = serverTime(cmd);
