...
void Animation::fake_animation(CUserCmd* cmd)
{
	...
	if (!init_fake_anim)
	{
		m_fake_state = static_cast<CCSGOPlayerAnimState*> (g_MemAlloc->Alloc(sizeof(CCSGOPlayerAnimState)));
	...
	g_LocalPlayer->SetAbsAngles(QAngle(0, m_fake_state->m_flGoalFeetYaw, 0));
	...
	g_LocalPlayer->SetAbsAngles(QAngle(0, m_server_abs_rotation, 0)); // restore real abs rotation
...
void Animation::real_animation(const bool send_packet, CUserCmd* cmd)
{
	...
	if (!init_real_anim)
	{
		m_real_state = static_cast<CCSGOPlayerAnimState*> (g_MemAlloc->Alloc(sizeof(CCSGOPlayerAnimState)));
	...
	if (send_packet)
	{
		...
		g_LocalPlayer->SetAbsAngles(QAngle(0, m_server_abs_rotation, 0));
